...
public class PoliticsFragment extends Fragment {
    ...
    private RecyclerView politicsListView;

    private SectionListObject politicsListObject;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        politicsListView = view.findViewById(R.id.politicsListView);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
        politicsListView.setLayoutManager(linearLayoutManager);

        swipeRefreshLayout = view.findViewById(R.id.swipeRefreshPoliticsFrag);
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                fetchPoliticsNews(view);
                swipeRefreshLayout.setRefreshing(false);
            }
        });

        progressBar = view.findViewById(R.id.politicsProgressBar);
        fetchingNews = view.findViewById(R.id.politicsFetchingTextView);

        fetchPoliticsNews(view);
        return view;
    }

    private void fetchPoliticsNews(View view) {
                ...
                (Request.Method.GET, "http://35.188.11.46:3000/results/politics?source=guardian", null, new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            ...
                            politicsListObject = new SectionListObject();
                            for (int i = 0; i < resultItems.length(); i++) {
                                JSONObject resultItem = (JSONObject) resultItems.get(i);

                                // Add Title and Long title
                                politicsListObject.newsTitleLong.add(resultItem.getString("webTitle"));
                                TruncateString truncateString = new TruncateString(resultItem.getString("webTitle"), WORD_LENGTH);
                                politicsListObject.newsTitle.add(truncateString.getTruncation());

                                politicsListObject.webURL.add(resultItem.getString("webUrl"));

                                //Add Time
                                DateToZoneTimeString dateToZoneTimeString = new DateToZoneTimeString(resultItem.getString("webPublicationDate"));
                                politicsListObject.newsTime.add(dateToZoneTimeString.getZoneTimeString());

                                //Add Section
                                politicsListObject.newsSection.add(resultItem.getString("sectionName"));

                                //Get image
                                String image = fetchImageURL(resultItem);
                                ImageDownloader imageDownloader = new ImageDownloader();
                                Bitmap img = imageDownloader.execute(image).get();
                                politicsListObject.newsImage.add(img);

                                //Get articleId
                                politicsListObject.articleId.add(resultItem.getString("id"));
                            }
                            if (politicsListObject.newsTitle.size() != 0) {
                                progressBar.setVisibility(View.GONE);
                                fetchingNews.setVisibility(View.GONE);
                            }
                            SectionCustomAdapter headlineCustomAdapter = new SectionCustomAdapter(getContext(), politicsListObject);
                            politicsListView.setAdapter(headlineCustomAdapter);
